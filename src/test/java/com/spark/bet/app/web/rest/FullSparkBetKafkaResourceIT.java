...
import com.spark.bet.app.config.KafkaProperties;
...
class FullSparkBetKafkaResourceIT {
    ...
    void setup() {
        ...
        FullSparkBetKafkaResource kafkaResource = new FullSparkBetKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/full-spark-bet-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/full-spark-bet-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
